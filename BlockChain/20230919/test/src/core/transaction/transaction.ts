...
//   데이터가 여러개인데, 하나의 문자열로 만든다
  serializeRow(row: TransactionRow){
    // 데이터 전달하기 위해서 단순히 문자열로 변환해주는 함수
  ...
  */
  createCoinbase(account : string, latestBlockHeight : number){
    // 채굴자일 경우 트랜잭션 해시와 서명이 없는데,
    // 보상을 받기 위한 트랙잭션이기때문에 아래 코드를 작성함.
  ...
  createTxOut(account : string, amount : number) : TxOut{
    ...
    console.log("길이??", account.length);
    if(account.length !== 42) throw new Error ("정상적인 주소가 아니다.");
...
export {Transaction, Receipt, Sender };

// 코드가 중요한게 아니고 흐름도가 중요함. 코드도 간략하게 이해해야하지만, 흐름도 이해가 중요!!

// 트랜잭션이 생성되면 트랜잭션 풀에 담기는 내용
// 트랜잭션의 내용은 in out이 있고
// in 에는 트랜잭션의 내용 out 내용에 대한 출력
