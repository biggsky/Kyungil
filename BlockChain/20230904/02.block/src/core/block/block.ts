...
class Block extends BlockHeader implements IBlock{
    ...
    static findBlock(generateBlock : Block){
        let hash : string;
        // nonce 변수는 블록의 채굴을 하는데 연산을 몇 번 진행했는지 값을 여기에 담을 것임
        let nonce : number = 0;
        ...
        while(true){
            ...
            // 난이도가 1이면은 2진값에 앞자리가 0이 1개이상이어야 하고,
            // 2면은 2진값에 0이 2개 이상이어야 한다.
            // 2진값으로 뽑아낸 값이 왜 difficulty랑 연관 이 있나요? => 컴퓨팅 연산을 넣고싶은데 개발자가 만든 내용. 그냥 문법...

            const binary : string = CryptoModule.hashToBinary(hash);
            console.log("바이너리값2: ", binary);
            ...
            if(result){
                ...
                console.log("완성된 해시값", generateBlock);
                // 완성된 블록을 내보내주자.
