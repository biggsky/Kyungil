...
export class Chain {
  ...
  getBlock(callbackFn: (block: Block) => boolean) {
    // const findBlock = this.chain.find((block: Block) => block.height === height);
  ...
  getBlockByHeight(height: number) {
    console.log("왓타임", height);
    // console.log("왓타임블록", Block.);
    return this.getBlock((block: Block) => block.height === height);
  }

  // 블록의 해시로 찾는 함수
  getBlockByHash(hash: string) {
    return this.getBlock((block: Block) => {
  ...
  getAdjustBlock() {
    ...
    const findHeight =
      height < this.INTERVAL ? 1 : Math.floor(height / this.INTERVAL) * this.INTERVAL;
  ...
  replaceChain(receivedChain: Block[]): Failable<undefined, string> {
    ...
    // 머클 루트, 해시값, 체인 전체 검증 등등의 로직이 더 추가되어 있을건데.
    ...
    if (latestReceivedBlock.height === 0)
      return {
        ...
        // 마지막블록이 곧 제네시스블록, 즉 코인이 만들어지지 않은 상태
      };
  ...
  */
  getAdjustmentBlock() {
    ...
    const getAdjustmentBlock: Block =
      this.length() < this.INTERVAL ? GENESIS : this.chain[currentLength - this.INTERVAL];

    // 최초 블록 or -10번째 블록 반환
    return getAdjustmentBlock;
  }

  // 전체적인 흐름도
  // 블록을 검증하는데는 높이나,해시나 등등
  // 머클이 어떤건지.
  // 상승이 어떻게 되는지
  // 체인에는 어떤 내용으로 조회를 하고 블록이 추가될때 제일 긴 체인이 정답인거랑.
  // 본인보다 길면 교체하는거
