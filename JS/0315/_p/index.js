...
// 배열의 길이가 변해도 length키값으로 배열의 길이를 가져올수 있다.
...
// const 한번 선언하면 값을 변경할수 없다. 재선언 또한 안된다.
// 상수의 값
// 객체는 키와 값이 있다.
...
let return4 = arr.find(function(i){
    ...
    // i 매개변수에
    console.log("item"+i);
    // retrun 값이 true 값이 반환되면 해당 아이템을 find 함수에서 반환해준다.
    return i === "딸기";
});
...
// 해당 배열을 반복시키면서 값을 찾고 그값의 인덱스를 반환해준다.
console.log(return5);
...
// filter 검색창 같은 알고리즘 구현할때 사용할거같다.

// 원하는 값을 모두 찾아서 반환해준다.
console.log(return6);

// 배열 메소드 map
let retrun7 = arr2.map(function(item){
    console.log(item);
    // return item;
    // 반환값들을 배열의 형태로 반환해준다.
    return item[0] === "이";
...
// ',' 값을 매개변수로 전달하면 문자열에서 ,의 문자를 잘라서 배열의
// 형태로 변경시켜준다.
